<template>
        ...
        <a-popover   trigger="hover" :open="hovered" @openChange="handleHoverChange" placement="bottomLeft">
            ...
            <a-popover title="任务详情" trigger="click" :open="clicked" @openChange="handleClickChange" placement="bottomRight" > 
                <template #content>
                    <div>
                        <a-descriptions size="small" :column="1" bordered>
                            ...
                            <a-descriptions-item v-if="TaskPopover_indata.task.deadline !== ''"
                                class="custom-descriptions-item" label="截止时间">
                                <EditTimeItem
                ...
                <div :style="{ 'max-width': tableUlWidth+'px' ,  overflow: 'hidden','text-overflow': 'ellipsis','white-space': 'nowrap',}"> {{ TaskPopover_indata.task.name }} </div>
